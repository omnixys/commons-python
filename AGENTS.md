<!-- repository: packages/python/commons | kind: PACKAGE | stack: python-package -->

# commons

> Repository development instructions for **commons** (Shared Package).
> Stack: python-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys shared platform contracts – transport-neutral DTOs, enums, errors, and models.

- Repository path: `packages/python/commons` (relative to the Omnixys root)
- Package: `omnixys-commons` (version: dynamic)
- Runtime: Python >=3.14 (uv)
- Kind: Shared Package

## Architecture

src/commons/ contracts (DTOs, enums, errors, models)
...
consumed by omnixys services and other packages; inspect consumers before breaking the public API
...
pytest (tests/ directory); ruff select=ALL; mypy strict with pydantic/sqlalchemy plugins

## Repository-Specific Rules

Zero- or minimal-dependency package; contracts are consumed by all Python services.
